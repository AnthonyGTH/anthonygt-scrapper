...
class BestBuyScraper:
    ...
    def __init__(self):
        self.base_url = "https://www.bestbuy.com.mx"
        self.site_name = "Best Buy"
    
    async def scrape_product(self, product_name: str, keywords: str) -> Optional[Dict[str, Any]]:
        """Scraper específico para Best Buy"""
        try:
            print(f"🛒 Scrapeando {product_name} en Best Buy...")
            ...
            async with async_playwright() as p:
                ...
                try:
                    # Buscar producto
                    search_url = f"https://www.bestbuy.com.mx/s/{keywords.replace(' ', '+')}"
                    ...
                    if not products:
                        print("❌ No se encontraron productos en Best Buy")
                        return None
                    ...
                    for product in products[:3]:
                        try:
                            ...
                            # Simular descuento
                            original_price = price * 1.25  # 25% de descuento
                            discount = 25.0
                            ...
                            print(f"✅ Best Buy: {title} - ${price:,.0f} ({discount:.1f}% descuento)")
                            return product_data
                            
                        except Exception as e:
                            print(f"⚠️ Error procesando producto Best Buy: {e}")
                            continue
                    
                    return None
                    
                except Exception as e:
                    print(f"❌ Error en Best Buy: {e}")
                    return None
                finally:
                    await browser.close()
                    
        except Exception as e:
            print(f"❌ Error general en Best Buy: {e}")
            return None
